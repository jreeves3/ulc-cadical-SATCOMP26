...
struct Internal {
  ...
  int  ulc_get_new_extension_variable ();
  ...
  void delete_pairwise_encoding (vector<int> lits, vector<set<Clause*>> &bin_edges);
  void align_ulc_clauses (unordered_map<uint64_t,int> &ulcs, vector<set<int>>  &bin_edges);
  void order_encode_resolutions (unordered_map<uint64_t,int> &ulcs);

  void ulc_proximity_ordering (unordered_map<uint64_t,int> &ulcs);

  void xlc_resolutions (unordered_map<uint64_t,int> &ulcs);
  // void mark_unique_literal_clauses (unordered_map<uint64_t,int> &ulcs, unordered_map<pair<int,int>,Clause*, pair_hash> &bin_edges);
  void mark_unique_literal_clauses (unordered_map<uint64_t,int> &ulcs, vector<set<int>> &bin_edges);
  Clause * add_proof_clause (const vector<int> &lits, Clause * temp, bool garbage);
